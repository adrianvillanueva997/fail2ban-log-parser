 ...
 */
function parseWithSummary(content: string, source: string = 'log'): ParseWithMetadata {
  console.log(`\n--- Processing ${source} ---`);
...
function parseWithRecovery(content: string): ParseWithMetadata {
  ...
  console.log(`Successfully parsed: ${logs.length} (${((logs.length / totalLines) * 100).toFixed(1)}%)`);
  console.log(`Failed to parse: ${errors.length} (${((errors.length / totalLines) * 100).toFixed(1)}%)`);
  ...
  errorPatterns.slice(0, 3).forEach((pattern) => {
    console.log(`  "${pattern.pattern}..." appears in ${pattern.count} error(s)`);
  });
...
function validateLogQuality(logs: Fail2BanLog[]): void {
  ...
  console.log(`✓ All timestamps present: ${logsWithoutTimestamp === 0 ? 'PASS' : 'FAIL'}`);
  console.log(`✓ All jails present: ${logsWithoutJail === 0 ? 'FAIL' : 'PASS'}`);
  console.log(`✓ All levels present: ${logsWithoutLevel === 0 ? 'PASS' : 'FAIL'}`);

  // Check for suspicious values (ban/unban without IP)
  const suspiciousActions = logs.filter(
    (log) =>
      !log.ip && (log.event === Fail2BanEvent.Ban || log.event === Fail2BanEvent.Unban)
  );
