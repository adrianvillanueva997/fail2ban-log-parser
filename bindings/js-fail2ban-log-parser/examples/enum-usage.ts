...
function getEnumName(value: number | undefined, mapping: Record<number, string>): string {
...
logs.forEach((log: Fail2BanLog) => {
  ...
  console.log(`  Header: ${headerName} | Event: ${eventName} | IP: ${log.ip || 'N/A'}`);
...
const bans = logs.filter((log): log is Fail2BanLog => log.event === Fail2BanEvent.Ban);
const unbans = logs.filter((log): log is Fail2BanLog => log.event === Fail2BanEvent.Unban);
const errorLogs = logs.filter((log): log is Fail2BanLog => log.level === Fail2BanLevel.Error);
